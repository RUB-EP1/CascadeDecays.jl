...
Compute the runtime kinematic input for one decay topology and one event from
external four-vectors. The result stores line invariant masses squared and the
local helicity angles used by amplitude evaluation.
"""
