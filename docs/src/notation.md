...
| vertex index | `1:nvertices` | Column in the incidence matrix: a binary decay node |
| `vertex` | — | Local amplitude payload (`VertexFunction`), **not** an index: `chain.vertices[v]` for vertex index `v` |
