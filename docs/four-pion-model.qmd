...
function format_cell(x)
    ...
    cell = sprint() do io
        show(IOContext(io, :compact => true, :limit => false), MIME("text/plain"), x)
    end
    return replace(cell, "|" => "\\|")
end
function show_table(df::AbstractDataFrame)
    cols = names(df)
    println("| ", join(cols, " | "), " |")
    println("| ", join(fill("---", length(cols)), " | "), " |")
    for row in eachrow(df)
        cells = [format_cell(getproperty(row, col)) for col in cols]
        println("| ", join(cells, " | "), " |")
    end
    return
end
...
```{julia}
#| label: two-body-resonance-table
#| output: asis
show_table(select(two_body, :r2 => :name, :jp, :mass, :width))
```

```{julia}
#| label: three-body-resonance-table
#| output: asis
...
transform!(
    cascade_ls,
    [:quantum, :r3_jp, :r2_jp] => ByRow() do quantum, r3_jp, r2_jp
        ...
        couplings = possible_vertex_couplings(row.topology, quantum, propagators)
        (;
...
transform!(
    pair_pair_ls,
    [:quantum, :r2a_jp, :r2b_jp] => ByRow() do quantum, r2a_jp, r2b_jp
...
dynamic_rows = crossjoin(
    select(subset(parents, :parent => ByRow(==("chi_c2"))), :parent, :quantum),
...
transform!(
    dynamic_rows,
    [:topology, :quantum, :propagators] => ByRow() do topology, quantum, propagators
        all_ls_decay_chains(topology, quantum, propagators)
    end => :chains,
)
transform!(
    dynamic_rows,
...
The first generated event illustrates the topology-local coordinates stored in
a `KinematicPoint` before the amplitude is evaluated.

```{julia}
#| label: kinematic-point-example
#| output: asis
first_point = KinematicPoint(task, first(events.momenta))
reference_kinematics = kinematics_at(first_point, task.reference_topology)

point_summary = DataFrame(
    field=["reference topology", "stored topologies", "line masses2", "vertex angles"],
    value=[
        "`" * replace(bracket_notation(task.reference_topology), "," => ", ") * "`",
        string(length(first_point.kinematics)),
        join(round.(reference_kinematics.line_masses2; digits=4), ", "),
        join(
            ["(cosθ=$(round(a.cosθ; digits=4)), ϕ=$(round(a.ϕ; digits=4)))" for a in reference_kinematics.vertex_angles],
            ", ",
        ),
    ],
)

show_table(point_summary)
```
